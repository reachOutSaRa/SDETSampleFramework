...
public class ExcelParser {
     ...
     * @lastModifiedDate 20210630
     */
    public static ArrayList<Row> findAllRowsByColumnValue(ParsedSheet sheet, String key, String value) {
        int index = sheet.indexOfKey(key);
        ...
        for ( int index = 0; index < parsedSheet.width;  index++)
        {
            ...
            switch (type) {
                ...
                case ARRAY_OBJECTS:
                    ...
                    // objectArray = Array of jsonObjects
                    //ArrayList objectArray = ExcelParser.<ArrayList<Object>>parseCellData(type, stringCellValue );
                    ...
                    ArrayList<Row> arrayRow = findAllRowsByColumnValue(parsedTargetSheet, targetKey, targetValue);
                    for (Row targetRow : arrayRow) {
                        jsonObject = parseRow(targetRow, parsedTargetSheet);
                        objectArray.add(jsonObject);
                    }

                    jsonRow.put( key, new JSONArray(objectArray) );
                    break;

                case REFERENCE:
                    // Split key to get real key, target sheet name and target column name
                    // Key example: monster@monsterSheet#monsterId
                    keyAndTarget = key.split(SIGN_TABLE_REFERENCE_SPLITTER);
                    key = keyAndTarget[0];

                    // Split sheet name and column name
                    realTarget = keyAndTarget[1].split(SIGN_SHEETNAME_COLUMNNAME_SPLITTER);
                    targetSheetName = realTarget[0];
                    targetKey = realTarget[1];
                    targetValue = getCellStringValue(cell);

                    targetSheet = parsedSheet.getSheet(targetSheetName);
                    parsedTargetSheet = new ParsedSheet(targetSheet.getWorkbook(), targetSheetName);
                    parsedTargetSheet.parseSheet();
    ...
    /**
     * Function to get cell String value
     * @param cell
     * @return
     * @author SaRa
     * @lastModifiedBy SaRa
